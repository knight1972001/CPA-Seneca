...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* m_vehicle) {
			return(m_vehicle->topSpeed() > 300);
		};
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* m_vehicle) {
			return(m_vehicle->condition().compare("broken") == 0);
		};
